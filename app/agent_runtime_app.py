...
# Initialize vertexai at import time to prevent AdkApp constructor from throwing 403 on project id resolution
try:
    vertexai.init(project=project_id, location=gemini_location)
except Exception as e:
    logging.warning(f"Could not initialize vertexai: {e}")
    ...
    def set_up(self) -> None:
        ...
        if os.environ.get("INTEGRATION_TEST") != "TRUE":
            try:
                ...
                logging.warning(
                    f"Could not initialize Google Cloud Logging client: {e}"
                )
